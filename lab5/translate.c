...
Tr_exp Tr_FieldVar(Tr_exp base, int offset) {
	return Tr_Ex(T_Mem(T_Binop(T_plus, unEx(base), T_Const(offset))));
}
...
Tr_exp Tr_CallExp(Temp_label name, Tr_level fun_level, Tr_level call_level, Tr_expList l) {
...
Tr_exp Tr_RecordExp(int n, Tr_expList fields) {
	...
	T_stm alloc = T_Move(T_Temp(rec), F_ExternalCall("initRecord",T_ExpList(T_Const(F_wordSize*n), NULL)));
	int i = n-1;
...
void Tr_procEntryExit(Tr_level level, Tr_exp body, Tr_accessList formals) {
	F_frag pf = F_ProcFrag(unNx(body), level->frame);
