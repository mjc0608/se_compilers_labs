...
// #endif
        ...
        case A_callExp: {
            ...
            if (!call || call->kind!=E_funEntry) {
                ...
                trans = Tr_CallExp(call->u.fun.label, call->u.fun.level, level, tr_args);
    ...
    switch(d->kind) {
        ...
        case A_functionDec: {
            ...
            for (fp=d->u.function; fp!=NULL; fp=fp->tail) {
                ...
                S_enter(venv, fp->head->name, E_FunEntry(func_level, func_label, tys_h, ret_ty));
                S_enter(block_venv, fp->head->name, E_FunEntry(func_level, func_label, tys_h, ret_ty));
            }
            for (fp=d->u.function; fp!=NULL; fp=fp->tail) {
                Ty_ty ret_ty;
                struct expty real_ret_ty_exp;
                ...
                real_ret_ty_exp = transExp(level, breakk, venv, tenv, fp->head->body);
                if (!ty_match(ret_ty, real_ret_ty_exp.ty)) {
                    EM_error(fp->head->pos, "procedure returns value");
                }
                Tr_procEntryExit(f->u.fun.level, real_ret_ty_exp.exp, ac_list);
                S_endScope(venv);
// #ifdef FUCK_THE_LAB
//                 frags = F_FragList(F_ProcFrag(NULL, NULL), frags); // fuck the lab
// #endif
